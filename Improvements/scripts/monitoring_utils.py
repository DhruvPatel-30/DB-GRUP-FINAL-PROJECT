...
def record_db_metrics(db_type, operation, start_time, error_count=0, mismatch_count=0):
    ...
    try:
        ...
        sql = """
            ...
            (db_type, operation, cpu_percent, mem_percent, avg_latency_ms, error_count, mismatch_count, created_at)
...
def print_metrics_summary():
    ...
    print(f"{'Database':<10} {'Operation':<15} {'Count':<8} {'Avg CPU':<10} {'Avg Latency':<12} {'Errors':<8} {'Mismatches':<12}")
    print("-"*90)
    
    for m in metrics:
        print(f"{m['db_type']:<10} {m['operation']:<15} {m['count']:<8} "
              f"{m['avg_cpu']:.1f}%{'':<6} {m['avg_latency']:.1f}ms{'':<6} "
              f"{m['total_errors']:<8} {m['total_mismatches']:<12}")
